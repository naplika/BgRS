    ...
    Public Module SecurityMeasurements
        ...
        public Function GenUniquePass() as string
            ...
            if runtimeinformation.IsOSPlatform(osPlatform.Windows) Then
                machineId = SharedElements.GetWindowsMachineId()
                diskUuid = SharedElements.RunCommand("wmic diskdrive get SerialNumber")
                cpuId = SharedElements.RunCommand("wmic cpu get ProcessorId")
            Elseif Runtimeinformation.IsOSPlatform(OSPlatform.Linux) Then
                machineId = SharedElements.RunBashCommand("cat /etc/machine-id")
                diskUuid = SharedElements.RunBashCommand("blkid -s UUID -o value $(df --output=source / | tail -1)")
                cpuId = SharedElements.RunBashCommand("cat /proc/cpuinfo | grep 'model name' | uniq")
            Elseif RuntimeInformation.IsOSPlatform(OSPlatform.OSX)
                machineId = SharedElements.RunBashCommand("system_profiler SPHardwareDataType | awk '/UUID/ { print $3; }'")
                diskUuid = SharedElements.RunBashCommand("diskutil info / | awk '/Volume UUID/ {print $3}'")
                cpuId = SharedElements.RunBashCommand("sysctl -n machdep.cpu.brand_string")
            End If
        ...
        public Function DecryptConfig() As String
            if File.Exists(SharedElements.Settingspath) Then
                dim str as string = io.File.ReadAllText(SharedElements.Settingspath)
                str = UnFuckString(str, GenUniquePass())
        ...
        public Function DecryptAuth() As String
            if File.Exists(SharedElements.loginpath) Then
                dim str as string = io.File.ReadAllText(SharedElements.loginpath)
                str = UnFuckString(str, GenUniquePass())
                str = str.Replace(vbcrlf, "").Replace(vbLf, "").Replace(vbCr, "").Replace("  ", "")
                return str
            Else
